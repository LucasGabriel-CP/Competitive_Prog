...
#define nil 0
    ...
    void build(int id, int l, int r, const std::vector<int> &vet){
        if(l == r) seg[id] = vet[l];
        else{
...
//graph vertex -> cost
std::vector<std::pair<int, int>> AdjList[maxn];
//Segtree
SegTree tree;
std::vector<int> arr;
//LCA
...
void HLD(int u, int p){
    ...
    position[u] = pos++;
    chain[u] = chainId;
    for (std::pair<int, int> v: AdjList[u]){
...
int queryUp(int from, int to){
    ...
    while(chain[cur] != chain[to]){
        ...
        ans = std::max(ans, tree.query(position[chainHead[chain[cur]]], position[cur]));
        cur = parent[chainHead[chain[cur]]][0];
    }
    //Modify the last answer
    ans = std::max(ans, tree.query(position[to]+1, position[cur]));
    return ans;
}

int query(int u, int v, int lca){
    int mx = queryUp(u, lca);
    mx = std::max(mx, queryUp(v, lca));
    return mx;
}
